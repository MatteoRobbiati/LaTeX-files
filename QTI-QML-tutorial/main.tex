...
\begin{frame}{A possible definition of ML}
    ...
    ML is an important sub-discipline of both \textbf{artificial intelligence} and \textbf{statistics}, which provides us with a set of tools (algorithms, models) through which \textbf{we can teach a computer} to perform certain tasks, such as pattern recognition, classification, regression, clustering, etc.
    \end{tcolorbox}
...
\pause
    ...
    \faArrowCircleRight\,\, If we want to implement a machine learning strategy, we need some basic elements:
    \begin{itemize}
...
\faArrowCircleRight\,\, Let's consider two variables $x$ and $y$ (respectively \textbf{input} and \textbf{output} variables), for which we know some theoretical relationship $f$ exists.
...
\begin{itemize}
    ...
    \pause
    \item[\tiny\faCircle] our theoretical estimation using a parametric model: $ y_{est} = \mathcal{M}(x | \bm{\theta})$;
...
To optimize the parameters of $\mathcal{M}$\footnote{We can choose the model we prefer.} so that $y_{est}$ and $y_{meas}$ are as close as possible\footnote{But without falling into over-learning!}.
\end{tcolorbox}
...
\large
    \faArrowCircleRight\,\, $\mathcal{J}$ is a function which quantifies the distance between $y_{meas}$ and $y_{est}$:
    $$\mathcal{J} \equiv \mathcal{J}\bigl[y_{meas}, y_{est}\bigr] = \mathcal{J}\bigl[y_{meas}, \mathcal{M}(x| \bm{\theta})\bigr].$$

\pause
\faArrowCircleRight\,\, Several $\mathcal{J}$ exist:
\begin{itemize}
    \item[\tiny\faCircle] \textbf{Mean Squared Error}: $ \mathcal{J}_{mse} \equiv N_{sample}^{-1}\sum_{i=1}^{N_{sample}} \bigl[y_{meas, i} - \mathcal{M}(x_i| \bm{\theta})\bigr]^2$;
